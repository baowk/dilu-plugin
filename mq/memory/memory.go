...
func (m *Memory) Publish(message mq.IMessager) error {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
	memoryMessage := new(mq.Message)
	...
	go func(gm mq.IMessager, gq queue) {
...
func (m *Memory) Register(name string, f mq.ConsumerFunc) {
	m.mutex.RLock()
	defer m.mutex.RUnlock()
	...
	go func(out queue, gf mq.ConsumerFunc) {
		var err error
		for message := range q {
			err = gf(message)
			if err != nil {
